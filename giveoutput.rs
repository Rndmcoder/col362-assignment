...
impl PartWriter {
    ...
    fn push_encoded<F: FnMut(u64, &[u8]) -> Result<()>>(
        &mut self, enc: &[u8], mut write: F,
    ) -> Result<()> {
        let bs     = self.buf.len();
        let usable = bs - 2;
        if enc.len() > usable {
            anyhow::bail!("encoded row ({} bytes) > usable block space ({})", enc.len(), usable);
        }
...
impl<'a, R: BufRead, W: Write> Executor<'a, R, W> {
    pub fn new(mut disk_in: R, disk_out: &'a mut W, ctx: &'a DbContext) -> Result<Self> {
        ...
        Ok(Self {
            disk_in, disk_out, block_size, ctx,
    ...
    pub fn set_memory_limit_mb(&mut self, mb: u32) {
        // Keep 20 MB headroom for runtime/stack/code.
    ...
    // ── Compute safe chunk capacity ───────────────────────────────────────────
    // bpr  = bytes per row (in memory, not encoded)
    // frac = fraction of data budget to use (e.g. 4 → use 1/4)
    // Hard cap: never exceed 32k rows regardless.
    fn safe_cap(&self, bpr: usize, frac: usize) -> usize {
        let budget = self.memory_limit_bytes / frac;
        (budget / bpr.max(1)).max(64).min(32_768)
    }
    ...
    fn read_block(&mut self, abs: u64) -> Result<Vec<u8>> {
        ...
        while n < bs {
            ...
            if a.is_empty() { anyhow::bail!("disk EOF block {} after {}/{}", abs, n, bs); }
    ...
    // Estimate in-RAM bytes per row.
    fn row_ram(row: &Row) -> usize {
        let p = std::mem::size_of::<usize>();
        3 * p + row.iter().map(|(k, v)| 6 * p + k.len() + v.len()).sum::<usize>()
    }
    ...
    fn emit_row(
        ...
        if *out_cap == 0 {
            // Use 1/4 of budget; cap at 32k rows.
            let bpr = Self::row_ram(&row).max(1);
            *out_cap = self.safe_cap(bpr, 4);
    ...
    fn ensure_on_disk(&mut self, rs: RunSet) -> Result<Vec<(u64, u64)>> {
        match rs {
            ...
            RunSet::InMemory(rows) => {
                ...
                let bpr = rows.first().map(|r| Self::row_ram(r)).unwrap_or(512);
                let cap = self.safe_cap(bpr, 4);
    ...
    fn collect_runset(&mut self, rs: RunSet) -> Result<Table> {
        let hint = rs.row_count().min(131_072);
        let mut out = Vec::with_capacity(hint);
        match rs {
            RunSet::InMemory(rows) => { out = rows; }
            RunSet::Spilled { runs, .. } => {
                let descs: Vec<(u64, u64)> = runs.iter()
                    .map(|r| (r.start_block, r.num_blocks)).collect();
                let bs = self.block_size as usize;
    ...
    fn exec_scan(&mut self, data: &ScanData) -> Result<RunSet> {
        ...
        for blk_rel in 0..file_blocks {
            ...
            'row_loop: for _ in 0..rc {
                if off >= bs - 2 { break; }
                let row_start = off;
                let (row, ok) = decode_table_row(b, &mut off, bs, &table_id, &col_specs);
                if !ok {
    ...
    fn exec_filtered_scan(&mut self, data: &FilteredScanData) -> Result<RunSet> {
        ...
        let (file_start, file_blocks) = self.get_file_range(&file_id)?;
        eprintln!("[FSCAN] '{}' start={} blocks={} preds={} proj={}",
                  table_id, file_start, file_blocks, preds.len(),
        ...
        for blk_rel in 0..file_blocks {
            ...
            'row_loop: for _ in 0..rc {
                ...
                let (row, ok) = decode_table_row(b, &mut off, bs, &table_id, &col_specs);
                if !ok {
                    eprintln!("[FSCAN] bad row blk={} off={}", file_start + blk_rel, row_start);
                    break 'row_loop;
                }
                ...
                let emit = if let Some(ref proj) = project {
                    ...
                    for (from, to) in proj {
                        match find_col(&row, from) {
                            ...
                            None    => eprintln!("[FSCAN] col '{}' missing", from),
                        }
                    }
                ...
                self.emit_row(emit, &mut out_buf, &mut out_runs, &mut out_cap, &mut out_total)?;
            }
        }

        let rs = self.finalize_output(out_buf, out_runs, out_total)?;
        eprintln!("[FSCAN] done: passed={} rejected={}", out_total, n_rejected);
        Ok(rs)
    }
    ...
    fn exec_filter(&mut self, data: &OptFilterData) -> Result<RunSet> {
        ...
        match input {
            RunSet::InMemory(rows) => {
                for row in rows {
                    if eval_preds(&row, &preds) {
                        self.emit_row(row, &mut out_buf, &mut out_runs,
        ...
        match input {
            RunSet::InMemory(rows) => {
                for row in rows {
                    ...
                    if !r.is_empty() {
                        self.emit_row(r, &mut out_buf, &mut out_runs,
                                      &mut out_cap, &mut out_total)?;
                    }
                }
            }
    ...
    fn exec_cross(&mut self, data: &OptCrossData) -> Result<RunSet> {
                ...
                for _ in 0..rc {
                    if let Some(row) = decode_row(&ibuf, &mut off) {
                        ...
                        if chunk_bytes + rb > chunk_budget && !chunk.is_empty() {
                            self.cross_emit_chunk(&chunk, &outer_descs,
                                                  &mut out_buf, &mut out_runs,
                                                  &mut out_cap, &mut out_total)?;
        ...
        if !chunk.is_empty() {
            self.cross_emit_chunk(&chunk, &outer_descs,
                                  &mut out_buf, &mut out_runs, &mut out_cap, &mut out_total)?;
        }
            ...
            for bi in 0..*i_nblk {
                ...
                for _ in 0..rc {
                    if let Some(row) = decode_row(&ibuf, &mut off) {
                        let rb = 3*p + row.iter().map(|(k,v)| 6*p+k.len()+v.len()).sum::<usize>();
                        if chunk_bytes + rb > chunk_budget && !chunk.is_empty() {
                            self.filter_cross_emit_chunk(
                                &chunk, &preds, &outer_descs,
                                &mut out_buf, &mut out_runs, &mut out_cap, &mut out_total)?;
                            chunk.clear(); chunk_bytes = 0;
                        }
                        chunk_bytes += rb;
    ...
    fn exec_hash_join(&mut self, data: &HashJoinData) -> Result<RunSet> {
        ...
        let p = std::mem::size_of::<usize>();
        eprintln!("[HASHJOIN] build={} probe={} budget={}MB",
                  build_count, probe_rs.row_count(), build_budget/(1024*1024));

        let mut build_map: HashMap<String, Vec<Row>> =
            HashMap::with_capacity(build_count.min(32_768));
        let mut build_bytes = 0usize;
        let mut overflowed  = false;

        let overflow_bpr = 512usize;
        let overflow_cap = self.safe_cap(overflow_bpr, 4);
        ...
        if overflowed {
            ...
            let map_cap = self.safe_cap(overflow_bpr, 4);
            ...
            let all_runs: Vec<ScratchRun> = map_runs.into_iter()
                .chain(overflow_runs.into_iter()).collect();
    ...
    fn grace_hash_join(
        ...
        let mut bw: Vec<PartWriter> = (0..n_parts)
            .map(|_| PartWriter::new(self.alloc_blocks(per_part_blks), bs))
            .collect();
            ...
            for bi in 0..*n_blk {
                ...
                for _ in 0..rc {
                    if let Some(row) = decode_row(&buf, &mut off) {
                        ...
                        enc.clear();
                        encode_row(&row, &mut enc);
                        let enc_copy = enc.clone();
                        bw[pi].push_encoded(&enc_copy, |abs, data| {
        ...
        for i in 0..n_parts {
            bw[i].finish(|abs, data| self.write_block_raw(abs, data))?;
        }
        let build_parts: Vec<(u64, u64, usize)> = bw.into_iter()
            .map(|w| (w.start_block, w.num_blocks, w.row_count)).collect();

        let probe_per_blks: u64 = per_part_blks * 10;
        let mut pw: Vec<PartWriter> = (0..n_parts)
            .map(|_| PartWriter::new(self.alloc_blocks(probe_per_blks), bs))
            .collect();
            ...
            for bi in 0..*n_blk {
                ...
                for _ in 0..rc {
                    if let Some(row) = decode_row(&buf, &mut off) {
                        ...
                        encode_row(&row, &mut enc);
                        let enc_copy = enc.clone();
                        pw[pi].push_encoded(&enc_copy, |abs, data| {
                            self.write_block_raw(abs, data)
                        })?;
                    }
                }
        ...
        for i in 0..n_parts {
            pw[i].finish(|abs, data| self.write_block_raw(abs, data))?;
        }
    ...
    //
    // The OOM bug was: bpr was estimated as 300 (a hardcoded default for
    // Spilled input), but actual rows were full-width table rows (~600+ bytes).
    // pass1_cap = budget/2 / 300 = ~76k rows × 600 bytes = 46 MB → OOM.
    //
    // Fix: sample the actual first row from the input to measure real bpr,
    // and use 1/3 of budget for sort chunks (leaving room for merge heads +
    // output buffer simultaneously).
    ...
    fn exec_sort(&mut self, data: &OptSortData) -> Result<RunSet> {
        ...
        let fits_ram = match &input {
            RunSet::InMemory(t) => rough_bytes(t) <= self.memory_limit_bytes * 3 / 4,
        ...
        // ── Measure actual bpr by reading the first row from input ────────────
        // This is the key fix: never hardcode bpr for Spilled inputs.
        let bpr = self.sample_bpr(&input)?;

        // ── Pass 1: sorted runs ───────────────────────────────────────────────
        // Budget: 1/3 of memory for the sort chunk.
        // This leaves 1/3 for merge heads (k * block_size) and 1/3 for output buf.
        let pass1_cap = self.safe_cap(bpr, 3);
        ...
        // ── Pass 2: k-way merge ───────────────────────────────────────────────
        // Output cap: 1/8 of memory. This is small — merge rows one at a time
        // so peak RAM = k*block_size + output_cap*bpr, both bounded.
        let merge_out_cap = self.safe_cap(bpr, 8);
        self.merge_sorted_runs(sorted_runs, &specs, merge_out_cap)
    }

    // Sample the first row from a RunSet to get a realistic bpr estimate.
    // Does NOT consume the RunSet (it reads from the spilled location).
    fn sample_bpr(&mut self, rs: &RunSet) -> Result<usize> {
        let p = std::mem::size_of::<usize>();
        let fallback = 512usize; // safe upper bound if sampling fails
        match rs {
            RunSet::InMemory(t) => {
                if t.is_empty() { return Ok(fallback); }
                Ok(Self::row_ram(&t[0]).max(64))
            }
            RunSet::Spilled { runs, .. } => {
                ...
                for run in runs {
                    ...
                    let mut off = 0usize;
                    if let Some(row) = decode_row(&buf, &mut off) {
                        let bpr = 3*p + row.iter()
                            .map(|(k,v)| 6*p + k.len() + v.len())
                            .sum::<usize>();
                        return Ok(bpr.max(64));
                    }
                }
                Ok(fallback)
            }
    ...
    fn merge_sorted_runs(
        &mut self,
        sorted_runs: Vec<ScratchRun>,
        specs:       &[SortSpec],
        out_cap:     usize,
    ) -> Result<RunSet> {
        let bs = self.block_size as usize;
        eprintln!("[SORT] merge_out_cap={} runs={}", out_cap, sorted_runs.len());
    ...
    // ── Helper: get file start block and block count ──────────────────────────
...
// Decode a single table row from a raw disk block buffer.
// Returns (row, ok). If ok is false, the row is partial/corrupt.
fn decode_table_row(
    b:        &[u8],
    off:      &mut usize,
    bs:       usize,
    ...
    let ok = ok && row.len() == col_specs.len();
    (row, ok)
}
...
fn sort_slice(tbl: &mut Table, specs: &[SortSpec]) {
    tbl.sort_unstable_by(|a, b| cmp_rows(a, b, specs));
}
...
fn opt_op_name(op: &OptQueryOp) -> &'static str {
    match op {
        ...
        OptQueryOp::FilteredScan(_) => "FilteredScan",
...
pub fn get_output_columns(op: &OptQueryOp, ctx: &DbContext) -> Vec<String> {
    match op {
        ...
        OptQueryOp::FilterCross(d) => {
            ...
            l.extend(get_output_columns(&d.right, ctx)); l
        }
        OptQueryOp::Cross(d) => {
            let mut l = get_output_columns(&d.left, ctx);
            l.extend(get_output_columns(&d.right, ctx)); l
